...
let APIKey = "c85cf78152914db2b360f3fe1ea7f9a8" // Ocp-Apim-Subscription-Key
...
class AzureFaceRecognition: NSObject {
    ...
    private func makePOSTRequest(url: String, postData: Data, headers: [String: String] = [:]) -> [AnyObject] {
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            if let data = data, let json = ((try? JSONSerialization.jsonObject(with: data, options: []) as? [AnyObject]) as [AnyObject]??), json != nil {
                object = json!
            }
            else {
